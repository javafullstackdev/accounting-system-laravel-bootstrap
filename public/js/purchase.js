$(document).ready( function(){
    ...
    $('#locations, #payment_status, #purchase_status').change( function(){
...
function update_purchase_entry_row_values( row ){
    if(typeof row != 'undefined'){
        ...
        // var currency_rate = parseFloat( $('option:selected', row.find('.currency_rate')).attr('data-currency_rate') );
...
function update_grand_total() {
    ...
    __write_number($('input.payment-amount'), 0.00, true);

    $('#payment_due').text(__currency_trans_from_en(grand_total, true, true));
...
$(document).on( 'click', 'button#submit_purchase_form', function(e){
    ...
    //         ref_no: {
    //             remote: {
    //                 url: "/payments/add_payment/check_bank_balance",
    //                 type: "post",
    //                 data: {
    //                     bank_id: function() {
    //                         return $( "#bank_id" ).val();
    ...
    //     messages: {
    //         ref_no: {
    //             remote: LANG.check_bank_balance
    //         }
    //     }
    // });
