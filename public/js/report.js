$(document).ready( function(){
    ...
    stock_expiry_report_table = $('table#stock_expiry_report_table').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'quantity_adjusted', name: 'quantity_adjusted'},
    ...
    $(document).on('click', '.recover_removed_items_btn', function(){
        swal({
            title: LANG.sure,
            icon: "warning",
            buttons: true,
...
function updateProfitLoss(){
    ...
    var loader = __fa_awesome();
    $('.opening_stock, .total_transfer_shipping_charges, .closing_stock, .total_sell, .total_purchase, .total_expense, .net_profit, .total_adjustment, .total_recovered, .total_sell_discount, .total_purchase_discount, .total_profits_sold_items').html(loader);
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $('.total_profits_sold_items').html(__currency_trans_from_en( data.total_profits_sold_items, true ));
