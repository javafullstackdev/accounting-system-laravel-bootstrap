$(document).ready( function(){
	...
	$('select#select_location_id').change(function(){
		$('input#selected_location_id').val($('select#select_location_id').val());
        ...
        $.ajax({
            method: "GET",
            url: "/sells/create-sale/get_inventory_products",

            data: {location_id: $('#select_location_id').val()},
            dataType: "html",
    ...
    $(document).on('change', '#row_currency_rate', function () {
        $.ajax({
            method: "GET",
            url: "/currencies/" + $('#row_currency_rate').val(),
            dataType: "json",
            success: function(result){
                $('input#exchange_rate').val(result['currency_rate']);
                $('#pos_table tbody').find('tr').each( function(){
                	var tr = $(this);
					__write_number(tr.find('input.pos_unit_price'), __read_number(tr.find('input.pos_unit_price_temp')) / result['currency_rate']);
					var line_total = __read_number(tr.find('input.pos_unit_price')) * __read_number(tr.find('input.pos_quantity'));
	...
	$('select#available_product_list').on('change', function() {
        ...
        if($('#pos_table tbody').find('tr').length){
        ...
        if(add_via_ajax){
            var purchase_id = $('select#avaiable_product_list').find(':selected').data('id');
            ...
            $.ajax({
                method: "GET",
                url: "/sells/pos/get_product_for_sale",

                data: { product_row: product_row, purchase_id: $('#available_product_list').val(), is_direct_sell: is_direct_sell },
                dataType: "json",
                success: function(result){
					if (result.success) {
                        $('input#location_id').val($('select#select_location_id').val());
                        ...
                        var currenced_unit_price = __read_number(this_row.find('input.pos_unit_price_temp')) * $('input#exchange_rate').val();
                        __write_number(this_row.find('input.pos_unit_price_temp'), currenced_unit_price);

                        __write_number(this_row.find('input.pos_unit_price'), __read_number(this_row.find('input.pos_unit_price_temp')) / $('input#exchange_rate').val());

                        var line_total = quantity * __read_number(this_row.find('input.pos_unit_price'));

                        __write_number(this_row.find('input.pos_line_total'), line_total, false, 2);
    ...
    $('table#pos_table tbody').on('change', 'select.currency_rate', function () {
        var tr = $(this).parents('tr');
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(result){
                var currenced_price = __read_number(tr.find('input.pos_unit_price_temp')) / result['currency_rate'];
                __write_number(tr.find('input.pos_unit_price'), currenced_price);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_temp', function () {
...
function pos_product_row(variation_id){
	...
	if(add_via_ajax){
		...
		$.ajax({
			method: "GET",
			url: "/sells/pos/get_product_row2/" + variation_id + '/' + location_id,
