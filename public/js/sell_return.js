$(document).ready( function(){
    ...
    $('#contact_id, #select_location_id').change( function(){
        get_customer_invoices();
    });

	$('#invoice_no').change( function () {
		$.getJSON("/products/sold_items_list", { location_id: $('#select_location_id').val(), tid: $('#invoice_no').val(), cid: $('#contact_id').val() }, function (retData) {
			var product_row = $('input#product_row_count').val();
            if ($('table#purchase_entry_table tbody').find("tr")){
                $('table#purchase_entry_table tbody').find("tr").remove();
            }
            var bank_id = retData['bank_id'];
            var bank_name = retData['bank_name'];
            var ui = retData['products'];
			...
			for (var i = 0; i < ui.length; i ++) {
				...
				$.ajax({
					method: "GET",
					url: "/sells/pos/get_product_row/" + variation_id + '/' + location_id + '/' + transaction_id,
...
function get_customer_invoices(){
    ...
    $.ajax({
        method: "POST",
        url: '/sell-return/get_customer_invoices',
        dataType: "html",
        data: { 'cid': cid, 'location_id': location_id },
        success: function(result) {
            if(result){
