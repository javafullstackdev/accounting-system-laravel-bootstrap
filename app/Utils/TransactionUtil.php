        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
	                ...
	                'purchase_lines_id' => $product['purchase_lines_id'],
...
//                'tax_id' => $product['tax_id'],
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($this->num_uf($payment['amount']) != 0) {
                    ...
                    if ($transaction->type == 'purchase') {
	                    ...
	                    $bank_balance = $this->num_uf($bank->balance);

	                    if ($bank_balance < $this->num_uf($payment['amount'])) {
		                    return false;
	                    }
                    }
    ...
    public function getPayments($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        $query1 = TransactionPayment::join(
            'transactions as t',
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $query1->whereBetween(DB::raw('date(paid_on)'), [$start_date, $end_date]);
        }

        $payments_supplier = $query1->get();
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $query2->whereBetween(DB::raw('date(paid_on)'), [$start_date, $end_date]);
        }

        $payments_customer = $query2->get();

        $payments['supplier'] = $payments_supplier->sum('amount');
        $payments['customer'] = $payments_customer->sum('amount');
...
//        $output['logo'] = ($il->show_logo != 0 && !empty($il->logo) && Storage::exists('public/invoice_logos/' . $il->logo)) ? config('app.url') . Storage::url('invoice_logos/' . $il->logo) : false;
//	    $output['logo'] = ($il->show_logo != 0 && !empty($il->logo) && Storage::exists('public/invoice_logos/' . $il->logo)) ? $il->logo : false;

	    $output['logo'] = Storage::url(config('constants.invoice_logo_path') . '/'.$il->logo);
...
//            $output['total_due'] = ($due == 0) ? 0 : $this->num_f($due, $show_currency);
    ...
    public function getProfitsSoldItems($business_id, $start_date = null, $end_date = null) {
        $query = TransactionSellLinesPurchaseLines::LeftJoin('purchase_lines as pl', 'pl.id', '=', 'transaction_sell_lines_purchase_lines.purchase_line_id')
            ->LeftJoin('transaction_sell_lines as tsl', 'tsl.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
            ...
            ->select(
                'sell_line_id',
                'purchase_line_id',
                DB::raw('((tsl.unit_price_inc_tax - pl.purchase_price_inc_tax) * transaction_sell_lines_purchase_lines.quantity) as profit')
            )->get();
	...
	public function getProfitsSoldItemsForReport($business_id, $start_date = null, $end_date = null, $location_id = null) {
		$query = TransactionSellLinesPurchaseLines::LeftJoin('purchase_lines as pl', 'pl.id', '=', 'transaction_sell_lines_purchase_lines.purchase_line_id')
			->LeftJoin('transaction_sell_lines as tsl', 'tsl.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
			->LeftJoin('transactions as t', 'tsl.transaction_id', '=', 't.id')
			->where('t.business_id', '=', $business_id)
		...
		$profit = $query->get()->sum('profit');
		return $profit;
	}

    public function getCostOfItemsSold($business_id, $start_date, $end_date) {
        $query = TransactionSellLinesPurchaseLines::LeftJoin('purchase_lines as pl', 'pl.id', '=', 'transaction_sell_lines_purchase_lines.purchase_line_id')
            ->LeftJoin('transaction_sell_lines as tsl', 'tsl.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
            ->LeftJoin('transactions as t', 'tsl.transaction_id', '=', 't.id')
            ->where('t.business_id', '=', $business_id)
            ...
            ->select(
                'sell_line_id',
                'purchase_line_id',
                DB::raw('(pl.purchase_price_inc_tax * transaction_sell_lines_purchase_lines.quantity) as cost')
            )->get();
    ...
    public function getTotalCustomerDebts($business_id, $start_date = null, $end_date = null) {
        $contacts = Contact::leftjoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
            ->where('contacts.business_id', $business_id)
            ->onlyCustomers()
            ->select(['contacts.id', 'contacts.name', 'contacts.type', 'contacts.city',
	            ...
	            DB::raw("
                    ...
                    + SUM(IF(t.type = 'sell', t.final_total, 0)) 
                as balances")
            ])->get();

        $totalBalances = $contacts->sum('balances');
        return $totalBalances;
    }

    /// Inventory Cost
    public function getTotalInventoryValue($business_id, $start_date, $end_date){
        $totalLeft = PurchaseLine::LeftJoin('transactions', 'transactions.id', '=', 'purchase_lines.transaction_id')
            ->where('transactions.business_id', '=', $business_id)
	        ->where('status', '=', 'received')
            ->select(
                DB::raw('((quantity - quantity_sold - quantity_adjusted) * purchase_price_inc_tax) as eachItemValue')
            );
	    if (!empty($start_date) && !empty($end_date)) {
		    $totalLeft->whereBetween('transactions.transaction_date', [$start_date, $end_date]);
	    }

        $total = $totalLeft->get();
        return $total->sum('eachItemValue');
    }

	public function getTotalInventory($business_id, $start_date, $end_date, $location_id){
    	if (!empty($location_id)) {
		    $totalLeft = PurchaseLine::LeftJoin('transactions', 'transactions.id', '=', 'purchase_lines.transaction_id')
			    ->where('transactions.business_id', '=', $business_id)
...
//        }
	    ...
	    if (!empty($filters['bank_id'])) {
		    $query->where('transactions.bank_id', $filters['bank_id']);
	    }
...
//	public function mapPurchaseSell($business, $transaction_lines, $mapping_type , $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
	        ...
	        if ($line->purchase_lines_id) $purchase_line_id = $line->purchase_lines_id;
...
*/
    ...
    public function createRegularBalanceTransaction($business_id, $contact_id, $amount, $regular_balance_id) {
        ...
        if ($amount > 0) {
            $selectedSells = $sells->where('transactions.payment_status', '!=', 'paid')->orderBy('transaction_date', 'asc')->groupBy('transactions.id')->get();
            $size = count($selectedSells);
            if ($size > 0) {
                foreach ($selectedSells as $selectedSell) {
                    $remaining = $selectedSell->final_total - $selectedSell->total_paid;
                    if ($amount > 0) {
	                    $payment_line['amount'] = $remaining <= $amount ? $remaining : $amount;
	                    $payment_line['method'] = 'other';
	                    $payment_line['paid_on'] = \Carbon::now()->toDateString();
	                    $payment_line['transaction_id'] = $selectedSell->id;
	                    $payment_line['created_by'] = auth()->user()->id;
	                    $payment_line['payment_for'] = $selectedSell->contact_id;
	                    $payment_line['parent_id'] = $regular_balance_id;
	                    TransactionPayment::create($payment_line);
	                    $this->updatePaymentStatus($selectedSell->id, $selectedSell->final_total);
                ...
                if ($amount > 0) {
                    ...
                    $selectedSells = $sells->where('transactions.payment_status', '=', 'paid')->orderBy('transaction_date', 'desc')->orderBy('invoice_no', 'desc')->groupBy('transactions.id')->get();
                    $size = count($selectedSells);
                    if ($size > 0) {
                        $selectedSell = $selectedSells[0];
...
//                        $payments = TransactionPayment::where('transaction_id', $selectedSell->id)->orderBy('paid_on', 'desc')->get();
//                        $payment = $payments[0];
//                        $input['amount'] = $payment->amount + $amount;
