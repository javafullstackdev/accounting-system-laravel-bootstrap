...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $currency_rate)
    {
        ...
        if ($currency_rate == 0 || $currency_rate == null) $currency_rate = 1.0;
        ...
        $variation_data = [
                ...
                'sub_sku' => $sku,
                'default_purchase_price' => $this->num_uf($purchase_price / $currency_rate),
//                'dpp_inc_tax' => $this->num_uf($dpp_inc_tax / $currency_rate),
	            'dpp_inc_tax' => $this->num_uf($purchase_price / $currency_rate),
                'profit_percent' => $this->num_uf($profit_percent),
                'default_sell_price' => $this->num_uf($selling_price / $currency_rate),
//                'sell_price_inc_tax' => $this->num_uf($selling_price_inc_tax / $currency_rate)
	            'sell_price_inc_tax' => $this->num_uf($selling_price / $currency_rate)
            ];
     ...
     * @return boolean
     */
    public function createVariableProductVariations($product, $input_variations, $currency_rate)
    {
        if (!is_object($product)) {
            $product = Product::find($product);
        }
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_data[] = [
                      ...
                      'product_id' => $product->id,
                      'sub_sku' => $sub_sku,
                      'default_purchase_price' => $this->num_uf($v['default_purchase_price'] / $currency_rate),
                      'dpp_inc_tax' => $this->num_uf($v['dpp_inc_tax'] / $currency_rate),
                      'profit_percent' => $this->num_uf($v['profit_percent']),
                      'default_sell_price' => $this->num_uf($v['default_sell_price'] / $currency_rate),
                      'sell_price_inc_tax' => $this->num_uf($v['sell_price_inc_tax'] / $currency_rate)
                    ];
                    $c++;
                }
     ...
     * @return boolean
     */
    public function updateVariableProductVariations($product_id, $input_variations_edit, $currency_rate)
    {
    ...
    public function getDetailsFromVariationForSellReturn($variation_id, $business_id, $location_id = null, $transaction_id, $check_qty = true) {
        ...
        $query2 = TransactionSellLine::where('transaction_id', '=', $transaction_id)
            ->where('product_id', '=', $products['product_id'])
            ->where('variation_id', '=', $variation_id)
            ->select('quantity')->first();
        $total_sold_quantity = $query2['quantity'];


        $transaction = Transaction::findOrFail($transaction_id);

        $query3 = PurchaseLine::join('transactions as t', 't.id', '=', 'purchase_lines.transaction_id')
            ->where('t.business_id', '=', $business_id)
            ->where('t.invoice_no', '=', $transaction->invoice_no)
            ->where('t.type', '=', 'sell_return');

        $total_returned_quantity = 0;
        if (!empty($query3)) {
            $query4 = $query3->where('product_id', '=', $products['product_id'])
                ->where('variation_id', '=', $variation_id)
                ->select(
            ...
            if (!empty($query4)) {
                $total_returned_quantity = $query4[0]->quantity;
            }
        }

        $products['qty_available'] = $total_sold_quantity - $total_returned_quantity;
        return $products;
    }
