            ...
            @if($loop->first)
                <tr>
                    <td>
                        ...
                        <div class="col-sm-12">
                          {!! Form::label('single_dpp', trans('product.exc_of_tax') . ':*', ['class' => 'hide']) !!}

                          {!! Form::text('single_dpp', @num_format($variation->default_purchase_price), ['class' => 'form-control input-sm dpp input_number', 'placeholder' => 'Please insert value', 'required']); !!}
                        </div>

                        <div class="col-sm-6">
                          {!! Form::label('single_dpp_inc_tax', trans('product.inc_of_tax') . ':*', ['class' => 'hide']) !!}
                        
                          {!! Form::text('single_dpp_inc_tax', @num_format($variation->dpp_inc_tax), ['class' => 'form-control input-sm dpp_inc_tax input_number hide', 'placeholder' => 'Including Tax', 'required']); !!}
                        </div>
                    </td>
