				...
				<div class="box-body">
					...
					<div class="col-sm-4">
						<div class="form-group">
							{!! Form::label('select_location_id', __('inventory.inventory') . ':*') !!}
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-map-marker"></i>
								</span>
								{!! Form::select('select_location_id', $business_locations, $transaction->location_id, ['class' => 'form-control select2 input-sm',
						...
						<div class="form-group">
							...
							<div class="input-group">
								...
								{!! Form::select('contact_id', 
									$contacts, $transaction->contact->id, ['class' => 'form-control mousetrap', 'id' => 'customer_id', 'placeholder' => 'Enter Customer name / phone', 'required']); !!}
					...
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
							...
							<div class="input-group">
								...
								{!! Form::text('transaction_date', @format_date($transaction->transaction_date), ['class' => 'datepicker form-control', 'readonly', 'required']); !!}
							</div>
						</div>
					</div>
					@php
					...
					@endphp
					<div class="hide @if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
					...
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
							{!! Form::label('bank_id', __('bank.bank') . ':*') !!}
							{!! Form::select('bank_id', $banks, $transaction->bank_id, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}
						</div>
					</div>
					<div class="col-sm-4">
						<div class="form-group">
							{!! Form::label('exchange_rate_list', __('purchase.p_exchange_rate') . ':*') !!}
							<div class="input-group">
							<span class="input-group-addon">
								<i class="fa fa-info"></i>
							</span>
								{!! Form::select('exchange_rate_list', $currencies, $default_currency_id, ['class' => 'form-control', 'required', 'id'=>'row_currency_rate']); !!}
								{!! Form::number('exchange_rate', $default_currency_rate, ['class' => 'form-control', 'id'=> 'exchange_rate', 'required', 'readonly']); !!}
								{{--<input type="number" id="exchange_rate" value="{{ $default_currency_rate }}">--}}
							</div>
					...
					<div class="col-sm-10 col-sm-offset-1">
						<div class="form-group">
							<div class="input-group">
								...
								<select class="form-control select2" id="available_product_list" placeholder="None">
									<option value="">None</option>
									@foreach($available_product_list as $list)
										@if($list->quantity > 0)
											<option value="{{ $list->id }}">{{ $list->name }} ($ {{ $list->dsp }} | {{ $list->quantity }} @if($list->quantity > 1) units @else unit @endif | EXP Date: {{ $list->exp_date }} )</option>
										@endif
									@endforeach
								</select>
								{{--{!! Form::select('available_product_list',--}}
									{{--array(), null, ['class' => 'form-control select2', 'id' => 'available_product_list', 'placeholder' => 'Select Product For Sale', 'required']); !!}--}}
							</div>
						</div>
						...
						<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
							...
							<tbody>
								@foreach($sell_details as $sell_line)
									...
									@include('sale_pos.product_row_for_sale', ['product' => $sell_line, 'row_count' => $loop->index, 'tax_dropdown' => $taxes])
								@endforeach
							</tbody>
						</table>
						</div>
