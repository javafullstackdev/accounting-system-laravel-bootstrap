<tr class="product_row" data-row_index="{{$row_count}}">
	...
	<td>
		<input type="text"  class="form-control pos_unit_price_temp input_number mousetrap" value="{{@num_format($product->default_sell_price)}}">
		<input type="hidden" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format($product->default_sell_price)}}">
	</td>
	<td class="{{$hide_tax}}">
	...
	<td>
		{!! Form::select('currency_country', $currencies, $default_currency_id, ['class' => 'form-control select2 currency_rate', 'required']); !!}
	</td>
